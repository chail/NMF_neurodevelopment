function plot_energy_neurodevelopment(t, num_subjects, plot_rows, ...
    plot_cols,start, stop)
% PURPOSE: plot boxplots for normalized energy of temporal coefficients
...
for ii = 1:num_components
    ...
    boxplot(energy(expr_order(ii),:), groups);
    [p,~, stats] = ranksum(energy(expr_order(ii),1:100), ...
        energy(expr_order(ii),101:200));
