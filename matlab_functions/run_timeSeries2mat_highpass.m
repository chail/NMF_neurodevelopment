function Aij = run_timeSeries2mat_highpass(timeSeriesInput,inputTR,windowSizeInput,windowOverlapInput,analysisTypeInput,outputDir,subjectNameInput,taskNameInput,timeLag)
...
if(nargin < 3 || isempty(windowSizeInput))
    disp(['Window size not provided, using length of time series: ',num2str(size(timeSeriesInput,2))]);
end

if(nargin < 4 || isempty(windowOverlapInput))
    windowOverlapInput = 0;
end

if(nargin < 5 || isempty(analysisTypeInput))
...
if(nargin < 7 || isempty(subjectNameInput))
    subjectNameInput = [];
end

if(nargin < 8 || isempty(taskNameInput))
    taskNameInput = [];
end

if(nargin < 9 || isempty(timeLag))
        timeLag = 1;
end
...
%% Save output file
if(analysisOutputFlag == 1)
    if(isempty('taskName'))%,'var'))
        outputfileName = fullfile(outputDir,['Aij_', subjectName, '_',analysisType,'_t',sprintf('%3.3d',windowSize),'_o',sprintf('%3.3d',windowOverlap),'.mat']);
    else
        outputfileName = fullfile(outputDir,['Aij_',subjectName, '_',taskName,'_',analysisType,'_t',sprintf('%3.3d',windowSize),'_o',sprintf('%3.3d',windowOverlap),'.mat']);
    end
else
    if(isempty('taskName'))%,'var'))
        outputfileName = fullfile(outputDir,['Aij_',subjectName,'_t',sprintf('%3.3d',windowSize),'_o',sprintf('%3.3d',windowOverlap),'.mat']);
    else
        outputfileName = fullfile(outputDir,['Aij_',subjectName, '_',taskName,'_t',sprintf('%3.3d',windowSize),'_o',sprintf('%3.3d',windowOverlap),'.mat']);
    end
end

save(outputfileName,'Aij','timeSeriesData','dataRange','dataLength','windowSize','windowOverlap','samplingRate','TR','N','analysisType','subjectName','taskName','-v6');
